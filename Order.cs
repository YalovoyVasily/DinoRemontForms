...
namespace DinoRemont
{
    public partial class Order : Form
    {
        ...
        String Query = DBConnection.qrOrder;
        ...
        private void Button_Add_Click(object sender, EventArgs e)
        {
            ...
            array.Add(TextBox_Login.Text);
            array.Add(cbPosition.SelectedValue);
            array.Add(cbAccount.SelectedValue);
        ...
        private void DataGridView_Employee_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                TextBox_Login.Text = DataGridView_Employee.Rows[e.RowIndex].Cells[4].Value.ToString();
                //Запись ID из таблицы в комбобокс
                cbPosition.SelectedIndex = Convert.ToInt32(DataGridView_Employee.Rows[e.RowIndex].Cells[6].Value) - 1;
        ...
        private void Button_Correct_Click(object sender, EventArgs e)
        {
            ...
            array.Add(cbPosition.SelectedValue);
            array.Add(cbAccount.SelectedValue);
            procedure.procedure_Execution("Order_update", array);
            Employee_Load(sender, e);
        }
        ...
        private void CheckBox_Filter_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                case (true):
                    dgvFill(Query + string.Format("where [Price_Order] like '%{0}%' " +
                                "or" + "[Material_Order] like '%{0}%' " +
                                "or" + "[Amount_Order] like '%{0}%' " +
                                "or [Status_Order] like '%{0}%'" +
                                "or [Name_Supplier] like '%{0}%'" +
                                "or [Last_Name_Employee] like '%{0}%'", TextBox_Search.Text), DataGridView_Employee);
                    break;
                case (false):
                    dgvFill(Query, DataGridView_Employee);
                    break;
